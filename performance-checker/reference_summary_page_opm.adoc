...
Le positionnement du curseur sur le nombre d'événements pour les événements actifs indique le type et le nombre d'événements. Les événements critiques sont affichés en rouge (image:../media/treemapred_png.gif["Icône de TreeMap – couleur rouge"]), et les événements d'avertissement sont affichés en jaune (image:../media/treemapstatus_warning_png.gif["Icône de TreeMap – État d'avertissement"]).
...
image::../media/iops_graph.gif[graphique iops]
